...
const handleProjectDelete = () => {
    if (projectToDelete.value) {
        actionForm.delete(route('projects.destroy', projectToDelete.value), {
            onSuccess: () => {
                closeModal();
            },
        });
...
const confirmProjectDeletion = (projectId) => {
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
                    ...
                    <div class="p-6 flex space-x-2" v-else v-for="project in projects" :key="project.id">
                        ...
                        <svg @click="confirmProjectDeletion(project.id)" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                ...
                <div class="mt-6 bg-white shadow-sm rounded-lg" v-if="invitations.length !== 0">
                    <div class="p-6">
                        <h1 class="font-bold text-xl">Incoming project invitations</h1>
                        <div class="mt-4 flex items-center space-x-4" v-for="invitation in invitations">
                            <p>{{ invitation.title }}</p>
                            <PrimaryButton @click="actionForm.put(route('invitations.update', invitation))">Accept</PrimaryButton>
                            <DangerButton @click="actionForm.delete(route('invitations.destroy', invitation))">Decline</DangerButton>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <Modal :show="confirmingProjectDeletion" @close="closeModal">
            <div class="p-6">
                ...
                <p class="mt-1 text-sm text-gray-600">
                    This action is irreversible. Deleting this project will permanently remove all
                    associated elements.
                </p>
                ...
                <div class="mt-6 flex">
                    ...
                    <DangerButton @click="handleProjectDelete" class="ms-3">Delete Project
                    </DangerButton>
                </div>
            </div>
        </Modal>
